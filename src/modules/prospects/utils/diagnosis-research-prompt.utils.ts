...
export function buildDiagnosisResearchPrompt(prospect: Prospect) {
  ...
  return [
    ...
    'Tu respuesta debe ser SOLO un JSON valido dentro de un unico bloque de codigo ```json```.',
    'No agregues explicaciones, notas, referencias, citas, markdown adicional ni texto fuera de ese bloque.',
    '',
    'Objetivo editorial:',
    ...
    '{ ...json limpio... }',
    '```',
    '- sin texto antes ni despues del bloque',
    '- sin comentarios dentro del JSON',
    '- sin referencias a fuentes, notas al pie ni aclaraciones fuera de las claves del schema',
    '',
     'Prospecto JSON:',
     payload,
   ].join('\n');
